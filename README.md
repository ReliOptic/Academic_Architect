...
긴 강의 스크립트나 학습 노트를 업로드하면, AI가 한 번에 요약해주는 대신 **자료를 작은 부분으로 쪼갠 뒤 학습자와 한 부분씩 같이 읽어가는** 학습 에이전트입니다. AI가 먼저 짧은 질문을 던지고, 학습자의 답을 보고 이해 수준을 가늠한 다음, 그 수준에 맞춰 설명합니다. "AI 튜터"보다 **"같이 공부하는 동료"** 에 가까운 학습 흐름을 만드는 것이 목표입니다.
...
3. **Deliver** — 가늠한 수준에 맞춰 설명합니다. 모르면 더 풀어서, 잘 알면 더 깊이.
...
4. 답을 입력하면 AI가 답변을 분석해 **L1(표면 탐색)** 로 판정 → 힌트 한 줄 → 다시 답하면 **L2(구조 파악)** 로 갱신됩니다.
5. 그 수준에 맞춘 설명이 도착합니다.
6. 자유롭게 질문하다 `"다음"` 으로 넘어갑니다.
...
|   | 일반 AI 챗봇 | Academic Architect |
|---|---|---|
| 시작 방식 | 학습자가 질문하면 답변 | AI가 먼저 예측·답변을 요구 |
| 설명 방식 | 일반적인 답변 | 학습자의 이해 수준을 측정한 뒤 그 수준에 맞춤 |
| 진행 단위 | 한 턴씩 | 세그먼트 단위 학습 루프 |
| 결과물 | 채팅 로그 | 깊이 시각화 + 마크다운 보고서 |
